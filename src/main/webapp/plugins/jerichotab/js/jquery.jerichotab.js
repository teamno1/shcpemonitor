...
$.extend($.fn, {
    ...
    initJerichoTab: function(setting) {
        ...
        function createJerichoTab() {
            ...
            */
            var jerichotab = $('<div id="'+opts.uniqueId+'" class="jericho_tab"><div class="tab_pages" ><div class="tabs" style="width:96.5%"><ul /></div></div><div class="tab_content"><div id="jerichotab_contentholder" class="content" /></div></div>')
                                            .appendTo($(opts.renderTo));
            
            //apply contentcss to the content
            ...
            $.fn.jerichoTab = {
                ...
                addTab: function(tabsetting) {
                    ...
                    var input = "";
                    if (ps.tabFirer != null) {
                        var obj = document.getElementById('jerichotab_' + tabsetting.id);
                        if(obj!=null){
                        	 var jerichotabindex = obj.value;
                        ...
                        input = "<input type=\"hidden\" id=\"jerichotab_" + tabsetting.id + "\" value=\""+curIndex+"\">";
                        //$('#jerichotab_' + tabsetting.id).val();
                    }
                    ...
                    */
                    /*var newTab = $('<li class="jericho_tabs tab_selected" style="width:0px"  id="jerichotab_' + curIndex + '" dataType="' + ps.data.dataType + '" dataLink="' + ps.data.dataLink + '">' +
	                                    ...
	                                    '<div class="tab_right">&nbsp;</div>' +
	                                '</li>')*/
                    var newTab = $('<li name="'+ps.name+'" class="jericho_tabs tab_selected" style="width:131px"  id="jerichotab_' + curIndex + '" dataType="' + ps.data.dataType + '" dataLink="' + ps.data.dataLink + '">' +
                            '<div class="tab_left" style="width:' + 131 + 'px"  >' +
                                (ps.iconImg == '' ? '' : '<div class="tab_icon" style="' + 'background-image:url(' + ps.iconImg + ')' + '">&nbsp;</div>') +
                                '<div class="tab_text" title="' + ps.title + '"  style="width:' + 110 + 'px"  >' + ps.title.cut(opts.tabWidth / 10 - 1) + '</div>  ' +
                                '<div class="tab_close">' + (ps.closeable ? '<a href="javascript:" title="关闭">&nbsp;</a>' : '') + '</div>' +
                                '<div class="tab_right">'+input+'&nbsp;</div>' +
                            '</div>' +
                        '</li>').appendTo($.fn.jerichoTab.tabpage).css('opacity', '0').applyHover().applyDbCloseEvent().
                        animate({ 'opacity': '1', width:'131px' }, 100, function() {$.fn.setTabActive(curIndex).loadData();
    	                }).applyCloseEvent().animate({ 'opacity': '1', width:'131px' }, 100, function() {
                    ...
                    tabHash.push({
                        ...
                        iframeId: 'jerichotabiframe_' + ps.code,
        ...
        $.fn.jerichoTab.resize = function() {
            ...
            if(explorer.indexOf("MSIE") >= 0){
            	$('#jerichotab_contentholder').height($("body").height() - opts.titleHeight-7);
                $(".jericho_tab iframe").height($("body").height() - opts.titleHeight-7);
            }else if (explorer.indexOf("Firefox") >= 0){
            	//alert($("body").height() - opts.titleHeight-fixHeight)
            	$('#jerichotab_contentholder').height($('body').height() - opts.titleHeight - 7);
                $(".jericho_tab iframe").height($('body').height() - opts.titleHeight - 7);
        ...
        $(window).resize(function() {
           // $.fn.jerichoTab.resize();
            ...
            //}
            if(explorer.indexOf("MSIE") >= 0){
            	$('#jerichotab_contentholder').height($("body").height() - opts.titleHeight-12);
                $(".jericho_tab iframe").height($("body").height() - opts.titleHeight-12);
            }else if (explorer.indexOf("Firefox") >= 0){
            	$('#jerichotab_contentholder').height($("body").height() - opts.titleHeight-12);
                $(".jericho_tab iframe").height($("body").height() - opts.titleHeight-12);
            }else if(explorer.indexOf("Chrome") >= 0){
    ...
    buildIFrame: function(src) {
        return this.each(function() {
           ...
           // localStorage.setItem('key',jerichotabiframe)
    ...
    loadData: function(flag) {
        return this.each(function() {
        	$('.jericho_tab .tab_selected').css('background', 'url(weblib/assets/images/gallery/tab1.jpg) repeat-x');
        	$('.jericho_tab .tab_selected').css('border-radius', '6px 6px 0 0');
        	$('.jericho_tab .tab_selected').css({'color':'#fff','border':'1px solid #7dc0db'});
        	$('.jericho_tab .tab_unselect').css({'background-color': 'url(weblib/assets/images/gallery/tab2.jpg) repeat-x','color': '#585858'});
        	//$('.jericho_tab .tab_unselect .tab_left').css({'background':'linear-gradient(to bottom, #ccc 0,#eee 60%,#fff 100%)','border-radius':'4px 0px 0px 0px','border-left':'1px solid #ccc','border-top':'1px solid #ccc'});
        	//$('.jericho_tab .tab_unselect .tab_right').css({'background':'linear-gradient(to bottom, #ccc 0,#eee 60%, #fff 100%)','border-radius':'0px 4px 0px 0px','border-right':'1px solid #ccc','border-top':'1px solid #ccc'});
                ...
                switch ($(me).attr('pos')) {
                    ...
                    case 'right':
                        ...
                        if (offLeft - $.fn.jerichoTab.tabWidth > -maxLeft)
                            ...
                            $.fn.jerichoTab.tabpage.animate({ left: -maxLeft-21 }, 100, function() {
        ...
        return this.each(function() {
            ...
            if (tabHash.length > 0 && offWidth > 0) {
                ...
                $.fn.jerichoTab.tabpage.css({ width: offWidth + $.fn.jerichoTab.tabPageWidth - ($.fn.jerichoTab.slidersWidth * 2) })
                		.animate({ left: -offWidth-21 }, 0, function() {
            ...
            var tabWidth = $.fn.jerichoTab.tabWidth+2;
            $.fn.jerichoTab.tabpage.width(tabHash.length * tabWidth);
        });
    },
    ...
    applyCloseEvent: function() {
        return this.each(function() {
            ...
            $('.tab_close>a', this).click(function(e) {
                ...
                var funcId=$(this).parent().parent().parent().attr('id');
                $("#"+funcId,parent.document).removeAttr('target');
                $("#"+funcId,parent.document).removeAttr('href');
                $("#"+funcId,parent.document).removeAttr('id');
               /* window.parent.document.getElementById(funcId).removeAttribute('target');
                window.parent.document.getElementById(funcId).removeAttribute('href');
                window.parent.document.getElementById(funcId).removeAttribute('id');*/
                /*$.cookie(funcId, '', {
    				expires : -1
    			});*/
            });
    ...
    applyDbCloseEvent: function() {
        return this.each(function() {
            var me = this;
            $('.tab_left>.tab_text', this).dblclick(function(e) {
                //prevents further propagation of the current event. 
                e.stopPropagation();
                if ($(this).length == 0) return;
                //remove tab from tabpageholder
                if($(this).attr("class")==''){
                	$(me).css({ 'opacity': '1', width: '131px' });
                }else{
                $(me).animate({ 'opacity': '0', width: '0px' }, 100, function() {
                    //make the previous tab selected whether the removed tab was selected
                	var lastTab = $.fn.jerichoTab.tabpage.children('li').filter('.tab_selected');
                 ...
                 $(".tabs").width('96.5%');
                 var aFuncId=$(this).parent().parent().attr('id');
                 $("#"+aFuncId,parent.document).removeAttr('target');
                 $("#"+aFuncId,parent.document).removeAttr('href');
                 $("#"+aFuncId,parent.document).removeAttr('id');
                /* window.parent.document.getElementById(aFuncId).removeAttribute('target');
                 window.parent.document.getElementById(aFuncId).removeAttribute('href');
                 window.parent.document.getElementById(aFuncId).removeAttribute('id');*/
            });
            ...
            switch ($.fn.jerichoTab.loader) {
                ...
                case 'righttag':
                    ...
                    if ($('#jerichotab_contentholder>.righttag').length == 0)
                        $('<div class="righttag" style="width:100px;">正在加载...</div>').appendTo($(this));
                    else
