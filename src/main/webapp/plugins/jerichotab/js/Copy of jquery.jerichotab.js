...
$.extend($.fn, {
                ...
                addTab: function(tabsetting) {
                    ...
                    */
                    /*var newTab = $('<li class="jericho_tabs tab_selected" style="width:0px"  id="jerichotab_' + curIndex + '" dataType="' + ps.data.dataType + '" dataLink="' + ps.data.dataLink + '">' +
	                                    ...
	                                    '<div class="tab_right">&nbsp;</div>' +
	                                '</li>')*/
                    var newTab = $('<li name="'+ps.name+'" class="jericho_tabs tab_selected" style="width:112px"  id="jerichotab_' + curIndex + '" dataType="' + ps.data.dataType + '" dataLink="' + ps.data.dataLink + '">' +
                            '<div class="tab_left" style="width:' + 105 + 'px"  >' +
                                (ps.iconImg == '' ? '' : '<div class="tab_icon" style="' + 'background-image:url(' + ps.iconImg + ')' + '">&nbsp;</div>') +
                                '<div class="tab_text" title="' + ps.title + '"  style="width:' + 90 + 'px"  >' + ps.title.cut(opts.tabWidth / 10 - 1) + '</div>  ' +
                                '<div class="tab_close">' + (ps.closeable ? '<a href="javascript:" title="关闭">&nbsp;</a>' : '') + '</div>' +
                            '</div>' +
                            '<div class="tab_right">'+input+'&nbsp;</div>' +
                        '</li>').appendTo($.fn.jerichoTab.tabpage).css('opacity', '0').applyHover().applyDbCloseEvent().
                        animate({ 'opacity': '1', width:'112px' }, 100, function() {$.fn.setTabActive(curIndex).loadData();
    	                }).applyCloseEvent().animate({ 'opacity': '1', width:'112px' }, 100, function() {
    ...
    loadData: function(flag) {
        return this.each(function() {
        	...
        	$('.jericho_tab .tab_selected').css('border-radius', '4px');
        	$('.jericho_tab .tab_selected').css('color', '#fff');
